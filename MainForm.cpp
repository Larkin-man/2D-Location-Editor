...
{
	int CellX, CellY;
	Out->MouseToCell(X,Y,CellX,CellY); //Получаем координаты ячейки
	if (CellX < 0 || CellY < 0)
		return; //Охуеть баг нажатия на скролл счиается -1
	if (CellX >= Map.Width||CellY >= Map.Height)
		return ShowMessage("Баг нет карты!");
	if (Button == mbLeft)
	{
		if (PanelSprites->Collapsed == false)
		{
			...
			{
				std::vector<TMap::TSprite>::iterator el,lishn=Map.Cell[CellX][CellY].Sprites.end();
				for (el = Map.Cell[CellX][CellY].Sprites.begin(); el != Map.Cell[CellX][CellY].Sprites.end(); ++el)
					if (X>=el->x&&Y>=el->y&&X<=el->x+Map.Sprites[el->Index]->Width&&Y<=el->y+Map.Sprites[el->Index]->Height)
						lishn = el;
				if (lishn != Map.Cell[CellX][CellY].Sprites.end())
				{
					if (MovedBtn->Down)
					{
					...
					Map.Cell[CellX][CellY].Sprites.erase(lishn);
				}
				else
				{
					if (CellY + 1 >= Map.Height)
						return;
					CellY++;
					Y -= 256;
               lishn=Map.Cell[CellX][CellY].Sprites.end();
					for (el = Map.Cell[CellX][CellY].Sprites.begin(); el != Map.Cell[CellX][CellY].Sprites.end(); ++el)
						if (X>=el->x&&Y>=el->y&&X<=el->x+Map.Sprites[el->Index]->Width&&Y<=el->y+Map.Sprites[el->Index]->Height)
							lishn = el;
					if (lishn != Map.Cell[CellX][CellY].Sprites.end())
               	...
               	if (MovedBtn->Down)
							SetSprite(lishn->Index);
						Map.Cell[CellX][CellY].Sprites.erase(lishn);
						OutDrawCell(Sender, CellX, CellY,
							TRect(Out->CellRect(CellX,CellY)), RangeState);
					}
            }
			}
			return;
		...
		if (CellBtn->Down || RangeBtn->Down)
			Painting = true;
		else if (MovedBtn->Down)
		{      //TODO:DDD
			Out->MouseToCell(X,Y,MoveX,MoveY);
      }
		UndoCurr = 0;
	...
	if (Button == mbMiddle)
	{
		...
		if (Map.Cell[X][Y].Image == Map.Variants[Map.Cell[X][Y].Image])
      	return;
		Map.Cell[X][Y].Image = Map.Variants[Map.Cell[X][Y].Image];
		//if (Map.Cell[X][Y].Image >= Map.TilesCount)
		//	Map.Cell[X][Y].Image = 0;
		Tiles->GetBitmap(Map.Cell[X][Y].Image, CurrTile); // тут получаешь нужное изображение по индексу из ImageList-а
			 ...
			 bool &CanSelect)
{
	...
	//Это для пограничных ячеек
	if (ARow >= Out->TopRow+Out->VisibleRowCount)
	{
		CanSelect = false;
		TRect rc = Out->CellRect(ACol, ARow);
		rc.Bottom = rc.Top + Out->DefaultRowHeight;
		OutDrawCell(Sender, ACol, ARow, rc, RangeState);
	}
	else
	if (ACol >= Out->LeftCol+Out->VisibleColCount)
	{
		CanSelect = false;
		TRect rc = Out->CellRect(ACol, ARow);
		rc.Right = rc.Left + Out->DefaultColWidth;
		OutDrawCell(Sender, ACol, ARow, rc, RangeState);
	}
}
//---------------------------------------------------------------------------
			 ...
			 int X, int Y)
{
	if	(Button == mbRight)
	{
		...
		if (X >= 0&&Y >= 0)
		{
			...
			//Log->Lines->Add(IntToStr(X)+" "+IntToStr(Y));
	}
}
//---------------------------------------------------------------------------
			 ...
			 TRect &Rect, TGridDrawState State)
{
	...
	int L,T,R,B,IND;
	if (Map.Cell[ACol][ARow].Sprites.size() > 0)
		for (std::vector<TMap::TSprite>::iterator el = Map.Cell[ACol][ARow].Sprites.begin(); el != Map.Cell[ACol][ARow].Sprites.end(); ++el)
		{
			//Log->Clear();
			...
			IND = el->Index;
			L = ceil(double(el->x) * Out->DefaultColWidth / TILESIZE);
			T = ceil(double(el->y) * Out->DefaultRowHeight / TILESIZE);
			L += Rect.left;
			T += Rect.top;
			R = Map.Sprites[el->Index]->Width * Out->DefaultColWidth / TILESIZE;
			B = Map.Sprites[el->Index]->Height * Out->DefaultRowHeight / TILESIZE;
			//Log->Lines->Add("ltwh"+IntToStr(L)+","+IntToStr(T)+","+IntToStr(R)+","+IntToStr(B));
			Out->Canvas->StretchDraw(TRect(L
			,T
			,L+R
			,T+B)
...
void __fastcall TEditorForm::OutClick(TObject *Sender)
{
	...
	if (MovedBtn->Down)
	{
		...
		Ruler = Out->ScreenToClient(Ruler);
		int x,y;
		Out->MouseToCell(Ruler.x,Ruler.y,x,y);
			 ...
			 int X, int Y)
{
	...
	{
		int mx, my;
		Out->MouseToCell(X,Y,mx,my);
		if (mx >= 0 && my >= 0)
			if (MoveX != mx || MoveY != my)
			{
				if (mx < MoveX && Out->LeftCol+Out->VisibleColCount < Out->ColCount)
					{ Out->LeftCol++; MoveX = mx+1; }
				else if (mx > MoveX && Out->LeftCol > 0)
						{ Out->LeftCol--; MoveX = mx-1; }
				if (my < MoveY&& Out->TopRow+Out->VisibleRowCount < Out->RowCount)
					{ Out->TopRow++; MoveY = my+1; }
				else if (my > MoveY && Out->TopRow > 0)
						{ Out->TopRow--; MoveY = my-1; }
...
////		Sprite->SetBounds(X-Sprite->Width,Y-Sprite->Height,Sprite->Width,Sprite->Height);
		...
		//Log->Lines->Add("refr"+IntToStr(X-w)+","+IntToStr(Y-h));
		RX = X-w;
		RY = Y-h;
		return;
	}
...
inline void TEditorForm::SetCell(int Col, int Row)
{
	if (Brush == bTile)
	{
		...
		if (CBTilesLog->Checked)
			Log->Lines->Add(IntToStr(Col)+" "+IntToStr(Row)+":"+IntToStr(SelTile));
...
void __fastcall TEditorForm::ScalerChange(TObject *Sender)
{
	...
	int cell = TILESIZE - Scaler->Position*scroler;
	Out->DefaultColWidth = cell;
	Out->DefaultRowHeight = cell;
	PixelOnMeter = (double)cell / METERS;
	if (PanelSprites->Collapsed == false && SelSprite >= 0)
		SetSprite(-1);
}
//---------------------------------------------------------------------------
